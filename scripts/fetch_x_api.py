#!/usr/bin/env python3
"""
X API v2から投稿を取得して、このアプリで読み込めるJSONとして保存するためのサンプルです。
...
例:
  python scripts/fetch_x_api.py --query "from:YamakawaTeruki" --limit 500 --out yamakawa.json
  python scripts/fetch_x_api.py --username YamakawaTeruki --limit 500 --out yamakawa.json
  python scripts/fetch_x_api.py --user-id 1234567890 --limit 500 --out user_tweets.json

注意:
  - APIプランによって使えるエンドポイント、期間、件数、レート制限が変わります。
  - 非公開・削除済み投稿や権限のない情報は取得できません。
  - bookmark_count / impression_count などは取得権限やエンドポイントに依存します。
"""
...
def fetch_pages(endpoint: str, bearer_token: str, params: dict[str, Any], limit: int, sleep_seconds: float) -> dict[str, Any]:
    ...
    while len(combined["data"]) < limit:
        ...
        combined["meta"] = {
            "result_count": len(combined["data"]),
